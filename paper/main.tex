...
and Network-Jacobian Amplification Analysis}
...
provable $y^6/144$ margin. The stability of the ML-augmented scheme is
characterized by the amplification factor of the closed operator, which
combines the base scheme's amplification with the network's \emph{own}
linearization---the Jacobian $J_{\mathrm{NN}}=\partial f_{\mathrm{NN}}/
\partial(	ext{stored history})$ of the learned correction---rather than
the analytic closure's; this amplification factor is differentiable and
serves both as an a priori diagnostic and as a training penalty. The closure assembles the
...
--- which we diagnose as a history-contamination loop---the network
differentiates its own contaminated history in time, an effect the
frozen-coefficient (analytic-closure) linearization cannot see---and which
the correct amplification factor, built on the network's Jacobian
$J_{\mathrm{NN}}$, is designed to penalize. We report the complete error, cost, and
...
closure modeling, IMEX schemes, Wiener filtering, amplification-factor stability,
...
correction, regularized by a differentiable penalty on the amplification
factor of the closed operator (built on the network's Jacobian $J_{\mathrm{NN}}$),
...
exact-flow and RK4 targets, and a differentiable amplification-factor
analysis of the \emph{closed} scheme---built on the network's own Jacobian
$J_{\mathrm{NN}}$---used as diagnostic and training penalty (Secs.~\ref{sec:stability}, \ref{sec:closure},
...
\item \emph{Amplification-factor penalty} on the network's Jacobian (derived in
...
\subsection{The amplification factor of the augmented scheme}
\label{sec:certificate}
The margin bound needs $|g_{\rm NN}|$ per mode \emph{for the actual trained
network}. Because the correction is a network, the per-mode amplification is
governed by the network's \emph{own} linearization---the Jacobian
\begin{equation}
J_{\mathrm{NN}}(\kappa)=\frac{\partial \hat f_{\mathrm{NN}}}
{\partial(\hat\omega^{\,n},\hat\omega^{\,n-1},\dots,\hat\omega^{\,n-S+1})},
\label{eq:jnn}
\end{equation}
the derivative of the learned correction with respect to its
\emph{stored-history} input, evaluated at the state the scheme actually
reaches. The closed-scheme amplification is the spectral radius of the
degree-$S$ companion whose top row combines the base AB2--CN2 linearization
with $-\,\mathrm{coef}\cdot J_{\mathrm{NN}}$ and whose remaining rows shift
the history:
\begin{equation}
G_{\rm eff}(\kappa)=\text{spectral radius of }
\begin{pmatrix}
a+b_0 & b_1 & \cdots & b_{S-1}\\
1 & 0 & \cdots & 0\\
 & \ddots & & \vdots\\
 & & 1 & 0
\end{pmatrix},\qquad
(b_0,\dots,b_{S-1})=(\text{base AB2 row})-\mathrm{coef}\cdot J_{\mathrm{NN}},
\label{eq:geff}
\end{equation}
which must be kept below unity. $J_{\mathrm{NN}}$ acts on the \emph{full}
stored history through the network's time-differencing stencils, so the
companion is of dimension $S$, not $2$: the instability the network can
create lives precisely in the history levels a two-level companion does not
contain. $G_{\rm eff}$ is differentiable in every trainable tensor (obtained
by automatic differentiation of the trained network at the operating state),
and serves both as a diagnostic---a per-step contraction certificate is
\emph{horizon-free}, removing the fixed-horizon asterisk of rollout training
structurally---and as the penalty $\mathcal L_{\rm stab}$ of
Sec.~\ref{sec:loss}. \TODO{$J_{\mathrm{NN}}$ amplification tests (B: truth
states; C: network-augmented states) in progress; the required unit test is
that $G_{\rm eff}$ reproduce the measured per-step rollout amplification
($\approx1.7$) before the penalty is re-enabled.}

\begin{remark}[Why the analytic-closure linearization is insufficient]
\label{rmk:frozen-insufficient}
A tempting simplification replaces $J_{\mathrm{NN}}$ by the \emph{analytic}
closure's linearization---Jacobians frozen to $i\sigma$ advection and the
time derivatives frozen to their exact relation $\hat\omega^{(j)}=\mu^{j}
\hat\omega$ (the Wiener freeze)---which collapses \eqref{eq:geff} to the
$2\times2$ companion derived in Appendix~\ref{app:vn}. By construction this
reproduces the amplification of the \emph{analytic} $R_3$ closure, which is
stable; it therefore reports $|G_{\rm eff}|\approx1$ irrespective of the
trained weights and cannot see the network's contribution. On the reference
model this frozen estimate reads valid-shell mean $0.9992$, yet the learned
closure's \emph{measured} per-step amplification in rollout is $\approx1.7$:
the frozen linearization measures the wrong operator. The learned-closure
amplification must be built on $J_{\mathrm{NN}}$ itself \eqref{eq:jnn}, not on
the analytic freeze. The history-contamination loop of Sec.~\ref{sec:results}
is exactly the mode this distinction exposes.
\end{remark}
...
pre-registered stability bars, ruling out the cheap inference-time fix.
Instrumentation of a single blow-up isolates the mechanism: a
\emph{history-contamination loop}. The network estimates $\Nddot$ by
finite-differencing its \emph{own} stored history in time; a fresh
correction perturbs the newest level, and that perturbation---uncorrelated
with the smooth part---is passed by the second difference at large gain and
re-injected, so the applied correction grows at a constant per-step factor
($\approx1.7$ in amplitude) while the field is still flat. The analytic arm,
which forms $\Nddot$ from the instantaneous field with no history, has no
such loop and is stable. This is a property of the augmented operator's
spectral radius through $J_{\mathrm{NN}}$ \eqref{eq:jnn}, and is the active
target.

\paragraph{The horizon wall.}
Extending the $5\times10^{-3}$ arm from 16 to 64 and 128 steps: \emph{0/10
draws stable}, blow-ups at steps 21--38, i.e.\ at
...
unconditional. This is the sharpest open problem, and it is a stability
question, not an accuracy one: the network reproduces the analytic closure to
$\approx1\%$ at every step, yet the closed loop diverges. The
amplification-factor penalty of Sec.~\ref{sec:certificate}, built on
$J_{\mathrm{NN}}$, is horizon-free by construction and targets exactly this;
crucially it must use the network's Jacobian, not the analytic-closure
linearization, which is blind to the loop (Remark~\ref{rmk:frozen-insufficient}).
A shallower $\Nddot$ stencil, which lowers the gain of the offending second
difference, is a complementary structural lever under test.
\figph{3.6cm}{Figure placeholder: blow-up step vs horizon, 10 draws, 16/64/128
steps, with the free-horizon line; $J_{\mathrm{NN}}$ amplification
$|G_{\rm eff}(\kappa)|$ vs.\ rollout step (tests B/C). Source:
\texttt{diagnostics/aposteriori\_stability.py},
\texttt{diagnostics/nn\_amplification.py}.}
...
learned scheme with an amplification-factor stability analysis. The theory side is complete and
...
(i) \emph{Amplification-factor--regularized training}: penalize the spectral
radius of the augmented operator built on the network's Jacobian
$J_{\mathrm{NN}}$ \eqref{eq:jnn} (validated by tests B/C to reproduce the
measured per-step rollout amplification), to convert the horizon-limited gain
into a per-step contraction guarantee; and the complementary structural lever
of a shallower $\Nddot$ time-difference stencil, which lowers the loop gain
directly.
...
\texttt{analysis/vn\_stability.py},
\texttt{Theoretical\_guarantees/imex\_stability\_regions.py}; amplification
factor ($J_{\mathrm{NN}}$): \texttt{diagnostics/nn\_amplification.py} (the
analytic-closure reduction of App.~\ref{app:vn}:
...
\section{Amplification of the analytic closure: the frozen-coefficient reduction}
\label{app:vn}

This appendix derives the frozen-coefficient amplification factor of the
closed scheme under the \emph{analytic}-closure linearization (the Wiener
freeze of Remark~\ref{rmk:frozen-insufficient}: time derivatives frozen to
$\hat\omega^{(j)}=\mu^{j}\hat\omega$, Jacobians to $i\sigma$ advection). This
is the amplification of the analytic $R_3$ arm---the stable control---and is
\emph{not} the amplification of the learned closure, which requires the
network Jacobian $J_{\mathrm{NN}}$ of Eq.~\eqref{eq:jnn}. In full generality:
lag depth $S$
...
visible inside this frozen reduction: the trainable parameters move
