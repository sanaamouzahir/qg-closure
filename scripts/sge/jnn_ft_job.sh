...
# race-safe idle-GPU reservation (gpu-idle-pick-race-safety)
if command -v nvidia-smi >/dev/null 2>&1; then
  ...
  echo "[jnn_ft] GPU $CUDA_VISIBLE_DEVICES on $HOSTNAME"
fi
...
python -u train_deriv_rollout.py \
  --deep-roots \
    data/ensemble_N5/FRC-kf4/forced_turbulence_dT_5em3 \
    data/ensemble_N5_7lag/FRC-256/forced_turbulence_dT_5em3 \
    data/ensemble_N5_7lag/FRC-b0/forced_turbulence_dT_5em3 \
    data/ensemble_N5_7lag/FRC-b05/forced_turbulence_dT_5em3 \
    data/ensemble_N5_7lag/FRC-b075/forced_turbulence_dT_5em3 \
    data/ensemble_N5_7lag/FRC-b1/forced_turbulence_dT_5em3 \
    data/ensemble_N5_7lag/FRC-b2/forced_turbulence_dT_5em3 \
  --strides 1,2,3 --init-ckpt "$CK" \
  --epochs 20 --lr 5e-5 \
  --vn-mode jnn --vn-lambda 0.5 --vn-single-slot --vn-developed-steps 0 \
  --anchor-lambda 0.03 \
  --run-name jnn_ft_v1
echo "[jnn_ft] TRAINER EXIT $?"
