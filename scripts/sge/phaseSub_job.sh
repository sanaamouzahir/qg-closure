#!/bin/bash
# phaseSub_job.sh - Generic SGE GPU worker for the FPSub (submarine) production run — mask+scenario added 2026-07-22 night
...
#        phaseSub_job.sh qg.grid.Nx=2048 qg.grid.Ny=2048 qg.time.save_rate=1080 \
...
if command -v nvidia-smi >/dev/null 2>&1; then
    ...
    echo "[phaseSub_job] selected GPU $IDLE_GPU on $HOSTNAME"
...
echo "[phaseSub_job] host $HOSTNAME  date $(date -u +%FT%TZ)"
echo "[phaseSub_job] cmd: python -u run_qg.py scenario=flow_past_submarine ${COMMON_OVERRIDES[*]} $*"
echo "----------------------------------------------------------------------"
...
python -u run_qg.py scenario=flow_past_submarine "${COMMON_OVERRIDES[@]}" "$@" &
SOLVER_PID=$!
...
if [ -n "$SCALARS_OUT" ]; then
    ...
    GUARD_PID=$!
    echo "[phaseSub_job] NaN-guard armed on $SCALARS_OUT (poll 300 s)"
else
    echo "[phaseSub_job] WARNING: no +qg.diag.out= arg -- NaN-guard disarmed"
fi
...
if [ "$RC" -ne 0 ]; then
    echo "[phaseSub_job] solver exited $RC (NaN-abort or crash) at $(date -u +%FT%TZ)"
    exit "$RC"
fi
...
echo "----------------------------------------------------------------------"
echo "[phaseSub_job] done at $(date -u +%FT%TZ)"
