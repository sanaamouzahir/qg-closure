...
OUT=$W/diagnostics/Results/apost_kcut_20260720
REP=$W/diagnostics/Results/apost_opt2_rep_20260711/FRC-kf4_ic912
CK=$D/rollout_ft_w31p3_certv2/best.pt
source $QG_ROOT/qg-env/bin/activate
cd $W/training
mkdir -p $OUT
for KC in 1.0 0.6; do
  TAG=kcut${KC/./p}
  echo "=== nn-kcut=$KC (fraction of the 2/3 cut ~170.7; 1.0 removes the annulus 171-241) ==="
  python -u rollout_aposteriori.py \
    ...
    --arms bare,closure --nn-kcut $KC --device cpu --nn-float64 \
    --out-dir $OUT --tag $TAG \
    --load-refs $REP/apost_refs_ic912_5em3_h64.npz || echo "RC_FAIL_$TAG"
done
echo "[kcut] done"
