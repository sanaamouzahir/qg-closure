...
def test_a_single_mode(out_dir: Path) -> None:
    ...
    # Helper: latexified scientific notation (1e-4 -> 10^{-4}, etc.)
    ...
    fields = [
        (r'$\dot N_{\rm analytical}$',          n_dot_an[0].cpu().numpy()),
        (r'$\dot N_{\rm empirical}$ (RK4)',     n_dot_fd[0].cpu().numpy()),
        (r'$\dot N_{\rm analytical} - \dot N_{\rm empirical}$',
                                                 (n_dot_an - n_dot_fd)[0].cpu().numpy()),
        (r'$\ddot N_{\rm analytical}$',         n_ddot_an[0].cpu().numpy()),
        (r'$\ddot N_{\rm empirical}$ (RK4)',    n_ddot_fd[0].cpu().numpy()),
        (r'$\ddot N_{\rm analytical} - \ddot N_{\rm empirical}$',
                                                 (n_ddot_an - n_ddot_fd)[0].cpu().numpy()),
    ]
    ...
    fig.suptitle(
        rf'Analytical vs Empirical $\dot N$, $\ddot N$, Single-Mode Check'
        '\n'
        rf'$\bar\omega(x,y,0) = {A1:.1f}\cos({k1:.0f}x) + {A2:.1f}\cos({k2:.0f}y)$, '
        rf'$N = {Nx}^2$, $L_x = L_y = 2\pi$, $dt_{{\rm FD}} = {_latex_sci(dt_fd)}$ (RK4 Reference)'
        '\n'
        rf'Rel Err $\dot N$ = ${_latex_sci(rel_err_dot.item())}$, '
        rf'Rel Err $\ddot N$ = ${_latex_sci(rel_err_ddot.item())}$',
        fontsize=11,
    )
    fig.tight_layout()
...
def test_b_2d_in_the_wild(
    ...
    # Helper: latexified scientific notation
    def _latex_sci(x: float) -> str:
        if x == 0:
            return r'0'
        from math import floor, log10
    ...
    ax.axhline(dt_fd**2, color='k', ls='--', alpha=0.5,
               label=fr'$\mathcal{{O}}(dt_{{\rm fd}}^2) = {_latex_sci(dt_fd**2)}$')
    ax.set_xlabel(r'$t$')
    ax.set_ylabel(r'Relative $L^2$ Error: Analytical vs $_{\rm FD}$')
    ax.set_yscale('log')
    ax.set_title(
        rf'Analytical vs Empirical $\dot N$, $\ddot N$, '
        rf'$dt_{{\rm fd}} = {_latex_sci(dt_fd)}$'
        '\n'
        r'(Case #2 Decaying Turbulence, $N = 256^2$, $L_x = L_y = 2\pi$, $T = 60$, Batch #0)'
    )
...
def test_c_truncation_error(
    ...
    fig.suptitle(
        r'AB2CN2 Analytical vs Empirical Time Discretization Closure '
        r'(Case #2 Decaying Turbulence, $N=256^2$, $L_x = L_y = 2\pi$, $T = 60$, Batch #0)'
        '\n'
    ...
    if len(diag_fields) > 0:
        ...
        # Helper: convert a float like 1e-4 or 5e-3 into a LaTeX-safe
        # "a \times 10^{b}" string, or just "10^{b}" when the mantissa is 1.
        def _latex_sci(x: float) -> str:
            if x == 0:
                return r'0'
            from math import floor, log10
            exp = int(floor(log10(abs(x))))
            mantissa = x / 10 ** exp
            # Round mantissa to 1 decimal; drop ".0" if integer
            m_str = f'{mantissa:.1f}'.rstrip('0').rstrip('.')
            if m_str in ('1', '-1'):
                sign = '-' if m_str.startswith('-') else ''
                return rf'{sign}10^{{{exp}}}'
        ...
        fig2.suptitle(
            rf'Pointwise Spatial Comparison at $\Delta T = {_latex_sci(diag_dT)}$ '
            rf'($K = {diag_K}$, $h_{{\rm fine}} = {_latex_sci(h_fine)}$)'
            '\n'
            r'(Case #2 Decaying Turbulence, $N = 256^2$, $L_x = L_y = 2\pi$, Batch #0)',
            fontsize=11,
        )
        fig2.tight_layout()
