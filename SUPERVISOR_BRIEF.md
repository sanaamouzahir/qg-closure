# Branch Supervisor brief — Time derivative as spatiotemporal convolution   (run this session as Opus 4.8)

You are Sanaa's colleague running the "Time derivative as spatiotemporal convolution" experiment
on branch `exp/time-conv`. You report to Sanaa and delegate to the shared team in .claude/agents/.

Launch: from this worktree, `claude --model opus`  (Opus 4.8).

## The idea
Generalize the time derivative from a pointwise-in-space linear combination over lags to a
per-order SPATIOTEMPORAL convolution: a spatial kernel PER LAG across the 7 time levels,
ω^(k) = Σ_i K_i^(k) * ω_{−i}, with the 1/dt^k scaling still analytic. This is the most expressive
form of the time-FD-as-Wiener idea (free-time-fd is its rank-0 / pointwise special case).
Init: Vandermonde × spatial delta, so init == control EXACTLY. Most expressive, most EXPENSIVE —
measure the cost (params, walltime, memory).
Success: beats `exp/free-time-fd` on Nddot by enough to JUSTIFY the parameter/compute increase.

## Status — BUILD AFTER free-time-fd's first result (NOT in parallel)
Do NOT start the model change until free-time-fd has landed a first Nddot number — this branch is
the more-expensive follow-on and its value is defined RELATIVE to free-time-fd, so it needs that
number to compare against. Until then: read code, scope the module, keep BRANCH_LOG updated.
...
- Init MUST be Vandermonde × spatial-delta so epoch-0 == control (otherwise the comparison is unclean).
- Keep the 1/dt^k scaling analytic; per-lag spatial kernels are DIMENSIONLESS (unit-dt) — dt-portability survives.
- MEASURE and report cost every run: params, GPU-h, peak memory, walltime — the success test is cost-justified gain.
- ORDER CLIP unchanged. No physics conditioning. GATE: don't submit until free-time-fd's first result exists.
...
**Nddot = 0.186 is the rollout ceiling** — but here the PRIMARY comparison is vs free-time-fd's Nddot,
cost-adjusted. Report both.
...
`<BRANCH>` = `time-conv` here | `GLOBAL` | `DATA`.
