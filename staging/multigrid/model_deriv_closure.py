...
class SpatialGrad(nn.Module):
    ...
    Spacing 1/dx, 1/dy is baked into the kernel, so outputs are true gradients.
    width=3 is the 2nd-order stencil; 5/7 are 4th/6th order, narrowing the
    finite-difference-vs-spectral gap on high-k fields at the cost of a slightly
    larger (still local) receptive field. Learnable kernels refine from there.

    The full (scaled) kernel is the Parameter -- Adam steps the physical operator
    directly. (Factoring into k0/dx + dimensionless correction makes Adam amplify
    the effective kernel step by 1/dx ~ 40x; dx portability is done at LOAD time.)
    """

    def __init__(self, channels: int, dx: float, dy: float,
        ...
        self.pad = width // 2
        # MULTIGRID: the stencil bakes 1/dx0 (the REFERENCE spacing it is built at).
        # On a grid of spacing dx_i the same learnable dimensionless operator gives
        # the true gradient as (conv)*(dx0/dx_i). We store dx0,dy0 for that rescale.
        self.dx0 = float(dx); self.dy0 = float(dy)
        kx = _central_diff_kernel('x', width, dx).repeat(channels, 1, 1, 1)
        ky = _central_diff_kernel('y', width, dy).repeat(channels, 1, 1, 1)
    ...
    def forward(self, x: torch.Tensor, dx: torch.Tensor = None,
        ...
        # MULTIGRID: per-sample dimensional rescale (dx,dy are (B,) tensors of the
        # batch's per-sample spacings). dx0/dx_i is a CONSTANT (no grad), so Adam
        # still steps the well-conditioned dx0 operator; on the reference grid it
        # is x1 -> bit-identical to single-grid. Handles mixed-dx batches (same
        # shape, different domain) because the rescale is per-sample, not per-batch.
        if dx is not None:
            gx = gx * (self.dx0 / dx).to(gx.dtype).view(-1, 1, 1, 1)
        if dy is not None:
            gy = gy * (self.dy0 / dy).to(gy.dtype).view(-1, 1, 1, 1)
        return gx, gy
...
class CheapDerivClosureNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, dt: torch.Tensor = None,
        ...
        dx_grad, dy_grad = self.grad(allf, dx=dx, dy=dy)   # MULTIGRID: per-sample rescale
        wx, px = dx_grad[:, :nt], dx_grad[:, nt:]   # omega-x grads, psi-x grads
        wy, py = dy_grad[:, :nt], dy_grad[:, nt:]
