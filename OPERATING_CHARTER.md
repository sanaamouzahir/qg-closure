...
things that wait for Sanaa shrinks; the set of things that fire in seconds
grows.
# CHARTER AMENDMENT v1.5 (append to OPERATING_CHARTER.md)

Supersedes I21(a)(b) of v1.4. Everything else in v1.4 (I22 path partition,
I23 digest logging, I24 reflex ladder, I25 session-open protocol) stands
unchanged and applies in BOTH modes.

Rationale: the constraint is not "agents must be local". The constraint is
"the shared front end must stay quiet while other people are working on
it". Outside those hours the cluster is the better host -- agents sit next
to the data, the logs, and the scheduler.

---

## 3. INVARIANTS -- replacing I21(a)(b), adding I26-I27

I21 EXECUTION MODEL -- TWO MODES.
    (a) DAY MODE (default; see I26 for hours): ALL agents run on Sanaa's
        local station. No agent process on the cluster, at any nice level.
        Cluster interaction is non-interactive ssh only, per I21(c):
            ssh mseas "<cd> && git pull --rebase && qsub ..."
            ssh mseas "qstat -u sanaamz"
            ssh mseas "tail -n 40 <log>"   |   scp mseas:<artifact> ./tmp/
        Canonical submission: PULL -> qsub(job) -> qsub(monitor,
        -hold_jid) -> report job ids.
    (b) NIGHT MODE: agents MAY run cluster-resident (front end), subject
        to I26 (hours + load veto + session cap + niceness). All other
        rules are identical in both modes -- same tiers, gates,
        templates, authorship, emails.
    (c),(d),(e) unchanged from v1.4.

I26 MODE RULES.
    (a) HOURS (America/New_York). NIGHT = 21:00-07:00 on weekdays, and
        all day Saturday/Sunday. DAY = everything else. Sanaa may declare
        either mode manually at any time; a manual declaration overrides
        the clock until revoked.
    (b) LOAD VETO (overrides the clock, always): before launching ANY
        cluster-resident session, check
            ssh mseas "uptime; who | wc -l"
        If 1-min load average > 2.0 OR interactive users > 4, DAY MODE
        applies regardless of hour. Re-check before each new session, not
        once per night.
    (c) SESSION CAP in NIGHT MODE: at most 3 concurrent agent processes
        on the front end (e.g. Fable + 2 branch supervisors). The 70%-CPU
        incident of 2026-07-13 was a fleet, not a session. Fable owns the
        count and refuses the 4th.
    (d) NICENESS is mandatory in NIGHT MODE, no exceptions:
            nice -n 19 ionice -c3 claude --model <model>
        ionice matters as much as nice: agent file-watching churn on
        /gdata contends with the metadata traffic of RUNNING GPU JOBS,
        not just with other humans.
    (e) tmux is mandatory for cluster-resident sessions (survives Sanaa's
        disconnects; keeps the session count visible and named).
    (f) Compute nodes have NO outbound network (verified 2026-07-13:
        api.anthropic.com unreachable from ibfdr-compute-0-2). Agents
        therefore cannot run there. If an outbound HTTPS proxy is ever
        provisioned for compute nodes, that becomes the preferred agent
        host in BOTH modes and this invariant is revisited.

I27 HANDOFF RITUAL (the two-host discipline).
    Exactly ONE host is the writer at any time. Alternating writers are
    safe; simultaneous writers are not.
    (a) CURFEW: a cluster-resident session starts no new task after
        06:30 and MUST exit by 07:00 (or immediately upon a load veto).
        "Exit" means: finish or abandon the current step, push, log,
        terminate the process -- not detach and leave running.
    (b) YIELD (cluster -> local, every morning): before exiting, the
        cluster session MUST
            1. commit and push ALL work (code, ledgers, docs -- in NIGHT
               MODE the cluster agent is the writer, so I22's write
               partition applies to JOB-generated digests only, not to
               the agent's own commits);
            2. write the night's entries into BRANCH_LOG.md and
               DECISIONS.md and push them;
            3. leave NOTHING uncommitted. If a push cannot complete
               (conflict, network), it FLAGs by email with the exact
               state and does not exit silently.
    (c) TAKE (local, on the next session): I25 applies -- pull first,
        read reports/*/status.md, ssh qstat, reconcile -- and
        additionally: read the night's BRANCH_LOG/DECISIONS entries
        before acting. Never assume the local clone is current.
    (d) EVENING HANDOFF (local -> cluster): the local session pushes
        everything before a night session may launch. Fable verifies
        `git status` clean + local HEAD == origin HEAD on both sides
        before starting NIGHT MODE work.
    (e) An uncommitted working tree on either host at handoff time is a
        protocol violation and the first agenda item of the next session.

## 1. DECISION RIGHTS -- night-mode note

NIGHT MODE does not widen the tiers. RED stays RED at 3am. What NIGHT
MODE changes is only latency: judgment that would have waited for Sanaa's
next local session (I24: "scientific failures wait") can be exercised by
a cluster-resident agent under its EXISTING authority -- I14 qdel+fix+
resubmit, YELLOW template submissions, GREEN diagnostics -- with the
[QG][ACTED] email arriving overnight for post-hoc ratification (24h rule,
counted from Sanaa's morning).

## 6. EMAIL PROTOCOL -- addition

6.5 A night session's first email of the night states the mode and the
    veto check result:
        MODE: NIGHT (load 0.20, users 2, 21:14 EST) | cap 1/3 sessions
    Its last email before curfew is a [QG][DIGEST][branch] handoff:
    what ran, what landed, what is pushed, what waits for the morning.

## 7. THE RATCHET -- addition

If a DAY-MODE ssh workflow repeatedly proves too slow for a recurring
task (many round-trips for one decision), that is a ratchet item: propose
either a batched ssh helper script (one call, many reads) or moving that
task class to NIGHT MODE. Do not silently violate DAY MODE to go faster.
