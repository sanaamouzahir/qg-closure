...
def main():
    ...
    p.add_argument('--lr-schedule', type=str, default='cosine',
                   choices=['cosine', 'plateau', 'none', 'warm_restarts',
                            'simulated_annealing'])
    p.add_argument('--patience', type=int, default=20,
                   help='for plateau schedule: epochs without improvement')
    p.add_argument('--warm-restart-T0', type=int, default=20,
                   help='initial cycle length (epochs) for warm_restarts / simulated_annealing')
    p.add_argument('--warm-restart-Tmult', type=int, default=2,
                   help='cycle-length multiplier between restarts (1=constant, 2=doubling)')
    p.add_argument('--lr-min', type=float, default=1e-6,
                   help='floor at the bottom of each cosine cycle')
    p.add_argument('--sa-damping', type=float, default=0.7,
                   help='for simulated_annealing: peak LR multiplier each restart (e.g. 0.7)')
    ...
    elif args.lr_schedule in ('warm_restarts', 'simulated_annealing'):
        # CosineAnnealingWarmRestarts: cosine schedule with periodic restarts
        # back to the peak LR.  T_0 = first cycle length, T_mult grows the cycle.
        # 'simulated_annealing' adds a per-restart damping factor on the peak,
        # so each "kick" is gentler than the last (classic SA temperature).
    ...
    # SA damping bookkeeping
    SA_damp = args.sa_damping if args.lr_schedule == 'simulated_annealing' else 1.0
    n_restarts_done = 0
    next_restart_epoch = args.warm_restart_T0  # epoch at which the first restart fires
    ...
    for epoch in range(args.epochs):
        ...
        if sched is not None:
            if args.lr_schedule == 'plateau':
                ...
                # CosineAnnealingWarmRestarts.step takes the "next epoch" index.
                sched.step(epoch + 1)
                if args.lr_schedule == 'simulated_annealing' and (epoch + 1) >= next_restart_epoch:
                    n_restarts_done += 1
                    new_peak = args.lr * (SA_damp ** n_restarts_done)
                    sched.base_lrs = [new_peak for _ in sched.base_lrs]
                    print(f"  [simulated_annealing] restart #{n_restarts_done} at "
                          f"epoch {epoch+1}, new peak LR = {new_peak:.2e}")
                    next_restart_epoch += args.warm_restart_T0 * \
                        (args.warm_restart_Tmult ** n_restarts_done)
            else:
                sched.step()
