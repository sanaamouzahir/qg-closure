...
Output: a directory at out_dir/<scenario>_dT_<tag>/ containing:
  ...
  samples/
    sample_000000.npz, sample_000001.npz, ...
        each .npz contains:
          omega_0, psi_0, grad_psi_sq, omega_x, omega_y       (Ny, Nx) float64
          omega_m1, omega_1_coarse, omega_K_fine              (Ny, Nx) float64
          e_total, e_anal_incr, e_NN_incr                     (Ny, Nx) float64
          f_NN_target, f_anal                                 (Ny, Nx) float64
...
# --------------------------------------------------------------------------- #
# Analytical N_dot, N_ddot via chain rule + bilinearity of J                  #
#                                                                             #
#   omega_t   = L*omega + N(omega),     N = -J(psi, omega) + F                #
...
# Forcing F is time-independent in our scenarios so Fdot = Fddot = 0.         #
# --------------------------------------------------------------------------- #

def compute_n_dot_analytical(omega: torch.Tensor, derivative,
                              L_hat: torch.Tensor,
                              F: Optional[torch.Tensor]) -> torch.Tensor:
...
def compute_n_ddot_analytical(omega: torch.Tensor, derivative,
                               L_hat: torch.Tensor,
                               F: Optional[torch.Tensor]) -> torch.Tensor:
    psi_hat = derivative.inv_laplacian * to_spectral(omega)
    psi = to_physical(psi_hat)
...
def compute_n_derivatives(omega, derivative, L_hat, F, max_order):
    """Return [N^(0), N^(1), ..., N^(max_order)] along the trajectory at `omega`.

    Static forcing (F^(k)=0 for k>=1). Mutually-recursive chain rule:
        omega^(k) = L omega^(k-1) + N^(k-1),      omega^(0) = omega
        psi^(k)   = inv_lap omega^(k)
        N^(m)     = -sum_{j=0}^m C(m,j) J(psi^(m-j), omega^(j))   (+F at m=0)

    Reduces to compute_n_dot_analytical / compute_n_ddot_analytical at orders
    1 and 2 (verified). N-derivatives are the local, learnable quantities; the
    L^k weighting (incl. the nonlocal beta term) is applied spectrally at
    inference, never learned.
    """
    omega_d = [omega]
...
class _OmegaLoader:
    ...
    Supports two source layouts:
      ...
      - .npz: a compressed archive with key 'omega_FR' (or 'omega') of shape
              ...
              once -- ~80 MB for 256^2 x 1200 x float64, fine.
...
def build_one_seed(
    ...
    arrays keyed by name, all numpy float64 ready for HDF5 dump.
    """
    seed = torch.tensor(seed_omega_np, dtype=dtype, device=device)[None]  # (1, Ny, Nx)

    # ---- 1) Ultra-fine RK4 forward by 3*Delta_T ---- #
    # We need (omega_m2, omega_m1, omega_n0) at three consecutive time levels
    # separated by Delta_T, so the NN can compute psi_dot, psi_ddot via finite
    # differences. The seed sits at t = -3*Delta_T relative to omega_n0.
    #
    #   t = 0           seed   (start of integration)
    #   t = Delta_T     -> save as omega_m2  (= seed + 1*dT)
    #   t = 2*Delta_T   -> save as omega_m1  (= seed + 2*dT)
    #   t = 3*Delta_T   -> save as omega_n0  (= seed + 3*dT)
    #
    # The coarse step and the K fine steps still operate from
    # (omega_m1, omega_n0) and produce omega_1_coarse / omega_K_fine at
    # t = 4*Delta_T relative to seed. omega_m2 is saved purely as an NN
    # input (for psi_ddot via 3-point finite diff in time).
    n_uf_to_dT  = int(round(Delta_T / h_ultrafine))
    n_uf_to_3dT = 3 * n_uf_to_dT
    omega_cur = seed.clone()
    omega_m2 = None
    omega_m1 = None
    for step in range(n_uf_to_3dT):
        omega_cur = rk4_step(omega_cur, h_ultrafine, derivative, L_hat, F_phys)
        if step + 1 == n_uf_to_dT:
            omega_m2 = omega_cur.clone()
        elif step + 1 == 2 * n_uf_to_dT:
    ...
    # ---- 5) NEW TARGET CONVENTION (Fix D): Delta_T-INDEPENDENT physics ---- #
    #
    # Old target was f = S^{-1}(Delta_T) * e_NN_incr, which mixed physics
    # with numerics (Delta_T, K, the AB2-CN2 response operator). At inference
    # we'd then multiply by Delta_T*S(Delta_T)/(...) and the S/S^-1 cancel.
    # Since the cancellation is exact, training the NN on f = S^{-1} * e_NN
    # is wasted complexity.
    #
    # NEW target: the BARE PHYSICS BRACKET (in coarse-fine sign convention,
    # i.e. matching E_analytical_phys' sign):
    #
    #     f_NN_target = (1/12) * [L*N_dot - 5*N_ddot]    (Ndot, Nddot at omega^0)
    #
    # By construction:
    #     e_NN_increment = -(Delta_T^3)(1 - 1/K^2) * f_NN_target
    #
    # so AT INFERENCE, the rollout correction is simply:
    #
    #     omega_corrected = omega_coarse + e_anal_incr - Delta_T^3*(1-1/K^2)*f_NN_pred
    #
    # NN-side advantages:
    #   - target is a PHYSICS quantity, independent of Delta_T and K.
    #   - same NN can be reused at any (Delta_T, K) (within the closure's
    #     regime of validity) by just rescaling at inference.
    #   - target magnitudes are O(1) physics, not O(Delta_T^3) numerics.
    #
    # Sign convention check:
    #   E_anal       has coarse-fine sign  (returns +1/12 * [L^3 omega + L^2 N])
    #   E_NN_bracket should have same sign: +1/12 * [L*Ndot - 5*Nddot]
    #   so f_NN_target = -e_NN_increment / coef = +1/12 * [L*Ndot - 5*Nddot] (target)
    # Raw N-time-derivatives are the LEARNED targets (R3+R4 needs Ndot, Nddot,
    # N3dot; N4dot provisioned for an optional R5). The L^k weighting (incl. the
    # nonlocal beta term) is assembled spectrally at inference, never learned.
    N_deriv        = compute_n_derivatives(omega_n0, derivative, L_hat, F_phys,
                                           max_order=4)
    Ndot_phys, Nddot_phys = N_deriv[1], N_deriv[2]
    N3dot_phys,  N4dot_phys = N_deriv[3], N_deriv[4]
    L_Ndot_phys    = L_op(Ndot_phys, L_hat)
    # Legacy single-channel R3 expensive bracket (kept for backward-compat /
    # ablation; the multi-channel design trains on the raw N-derivatives above).
    f_NN_target_phys = (1.0 / 12.0) * (L_Ndot_phys - 5.0 * Nddot_phys)

    # Verify analytically that f_NN_target ~~ -e_NN_increment / coef
    # (this should hold up to the order O(Delta_T^4) terms left out by Taylor
    #  truncation of the modified equation analysis -- so they won't match
    #  exactly but should be close in scale).
    f_NN_target_from_e = -e_NN_increment_phys / coef

    # f_anal = the analytical bracket (also coarse-fine sign):
    #     f_anal = (1/12) * [L^3 omega + L^2 N]
    # By construction f_anal = E_anal_phys, so just save that.
    f_anal_phys = E_anal_phys.clone()

    # ---- 6) Inputs / features for the NN ---- #
    # Streamfunction at all three time levels. psi = inv_lap * omega is a
    # single spectral multiplication + inverse FFT each. At inference time
    # these are essentially free: psi_n0 is needed by the AB2 step anyway,
    # psi_m1 is buffered from the previous step, and psi_m2 requires one
    # extra inverse-Laplacian per coarse step on the buffered omega_m2.
    psi_hat_n0 = derivative.inv_laplacian * qh_n0
    psi_n0     = to_physical(psi_hat_n0)
    psi_hat_m1 = derivative.inv_laplacian * qh_m1
    psi_m1     = to_physical(psi_hat_m1)
    qh_m2      = to_spectral(omega_m2)
    psi_hat_m2 = derivative.inv_laplacian * qh_m2
    psi_m2     = to_physical(psi_hat_m2)

    # Velocity components at t=0 (kept for backward compat / diagnostics)
    u_n0 = to_physical(-1 * derivative.dy * psi_hat_n0)
    v_n0 = to_physical(+1 * derivative.dx * psi_hat_n0)
    grad_psi_sq = u_n0 ** 2 + v_n0 ** 2
    # Vorticity gradients (kept for backward compat)
    omega_x = to_physical(derivative.dx * qh_n0)
    omega_y = to_physical(derivative.dy * qh_n0)

    # NEW: analytical input scaffolding (kept for diagnostics / ablations
    # only; the v2 minimal architecture does NOT use these as inputs).
    N_n0_phys      = -1.0 * J_phys(psi_n0, omega_n0, derivative)
    if F_phys is not None:
        N_n0_phys = N_n0_phys + F_phys

    L_omega_phys   = L_op(omega_n0, L_hat)
    qh_L_omega     = to_spectral(L_omega_phys)
    L2_omega_phys  = to_physical(L_hat * qh_L_omega)            # = L^2 omega
    L3_omega_phys  = to_physical(L_hat * to_spectral(L2_omega_phys))   # = L^3 omega

    L_N_phys       = L_op(N_n0_phys,  L_hat)
    L2_N_phys      = to_physical(L_hat * to_spectral(L_N_phys)) # = L^2 N

    def cpu32(t: torch.Tensor) -> np.ndarray:
        return t[0].detach().to('cpu', dtype=torch.float64).numpy()

    def cpu64(t: torch.Tensor) -> np.ndarray:
        # Use for fields where we need to preserve the actual magnitude --
        # specifically, the e_total/e_NN_incr/f_NN_target_from_e residuals,
        # which can be ~ DT^3 ~ 1e-9 in magnitude. float64 storage of such
        # small numbers is fine in isolation, but if you ever subtract two
        # of these stored fields you'll hit cancellation noise.
        return t[0].detach().to('cpu', dtype=torch.float64).numpy()

    return dict(
        omega_0          = cpu32(omega_n0),
        omega_m1         = cpu32(omega_m1),
        omega_m2         = cpu32(omega_m2),                # NEW (v2)
        psi_0            = cpu32(psi_n0),
        psi_m1           = cpu32(psi_m1),                  # NEW (v2)
        psi_m2           = cpu32(psi_m2),                  # NEW (v2)
        # Backward-compat / diagnostic fields
        ...
        # Residual / closure-validation fields: store as float64
        # because their actual magnitude can be near float64 epsilon.
        e_total          = cpu64(e_total_phys),
        e_anal_incr      = cpu64(e_anal_increment_phys),
        e_NN_incr        = cpu64(e_NN_increment_phys),
        f_NN_target      = cpu32(f_NN_target_phys),  # O(1), float64 fine
        f_anal           = cpu64(f_anal_phys),
        # Diagnostic-only Fix D scaffolding (not used by v2 minimal model)
        N_0              = cpu32(N_n0_phys),
        N_dot_0_anal     = cpu32(Ndot_phys),
        N_ddot_0_anal    = cpu32(Nddot_phys),
        N_3dot_0_anal    = cpu64(N3dot_phys),   # NEW (R4); keep float64 (high-k)
        N_4dot_0_anal    = cpu64(N4dot_phys),   # NEW (R5, provisioned)
        L_omega          = cpu32(L_omega_phys),
        L2_omega         = cpu32(L2_omega_phys),
        L3_omega         = cpu32(L3_omega_phys),
        L_N              = cpu32(L_N_phys),
        L2_N             = cpu32(L2_N_phys),
        f_NN_target_from_e = cpu64(f_NN_target_from_e),
    )
...
PER_SAMPLE_FIELDS = [
    # Core inputs for the v2 minimal architecture (3 omega + 3 psi snapshots)
    'omega_0', 'omega_m1', 'omega_m2',
    'psi_0',   'psi_m1',   'psi_m2',
    # Backward-compat / diagnostic fields
    'grad_psi_sq', 'omega_x', 'omega_y',
    'omega_1_coarse', 'omega_K_fine',
    'e_total', 'e_anal_incr', 'e_NN_incr',
    'f_NN_target', 'f_anal',
    # Fix D analytical scaffolding (saved for ablation/diagnostic only;
    # the minimal v2 architecture does NOT use these as inputs).
    'N_0', 'N_dot_0_anal', 'N_ddot_0_anal',
    'N_3dot_0_anal', 'N_4dot_0_anal',
    'L_omega', 'L2_omega', 'L3_omega',
    'L_N', 'L2_N',
    'f_NN_target_from_e',
]
...
def main():
    ...
    dtype = torch.float64 if args.dtype == 'float64' else torch.float64
    device = args.device
    ...
    # ----------------------------- Forcing ----------------------------- #
    # The N-derivative chain rule assumes STATIC forcing (Fdot = Fddot = 0), so
    # we build only time-independent cosine forcing and refuse C!=0 / F!=0.
    fc = _yaml_get(['qg', 'forcing']) or _yaml_get(['forcing'])
    F_phys = None
    forcing_meta = None
    if fc and isinstance(fc, dict) and fc.get('function') == 'unscaled_cosine':
        ...
        if Cc != 0.0 or Ff != 0.0:
            sys.exit("ERROR: time-dependent forcing (C!=0 or F!=0) is incompatible "
                     "with the static-forcing closure derivation (Fdot/Fddot != 0).")
        # Match forcing.py's grid convention exactly (linspace, endpoint=True).
        x = torch.linspace(0, Lx, Nx, device=device, dtype=dtype)
        y = torch.linspace(0, Ly, Ny, device=device, dtype=dtype)
        X, Y = x[None, :], y[:, None]
        F_phys = (A * torch.cos(Bk * X) + D * torch.cos(E * Y))[None]  # (1, Ny, Nx)
        forcing_meta = dict(function='unscaled_cosine', A=A, B=Bk, C=Cc, D=D, E=E, F=Ff)
        print(f"[build_training_data] static forcing {A}*cos({Bk}x)+{D}*cos({E}y); "
              f"|F|_rms={float(torch.sqrt((F_phys**2).mean())):.4e}")
    else:
        print("[build_training_data] no forcing (F = 0)")
    ...
    for b_pos, b in enumerate(batches):
        ...
        for j, s_idx in enumerate(seed_indices):
            ...
            np.savez_compressed(
                sample_path,
                # Core inputs for v2 minimal architecture: 3 omega + 3 psi snapshots
                omega_0=rec['omega_0'],
                omega_m1=rec['omega_m1'],
                omega_m2=rec['omega_m2'],                  # NEW (v2)
                psi_0=rec['psi_0'],
                psi_m1=rec['psi_m1'],                      # NEW (v2)
                psi_m2=rec['psi_m2'],                      # NEW (v2)
                # Backward-compat / diagnostic fields
                ...
                f_anal=rec['f_anal'],
                # Fix D scaffolding (diagnostic only)
                N_0=rec['N_0'],
                N_dot_0_anal=rec['N_dot_0_anal'],
                N_ddot_0_anal=rec['N_ddot_0_anal'],
                N_3dot_0_anal=rec['N_3dot_0_anal'],
                N_4dot_0_anal=rec['N_4dot_0_anal'],
                L_omega=rec['L_omega'],
                L2_omega=rec['L2_omega'],
                L3_omega=rec['L3_omega'],
                L_N=rec['L_N'],
                L2_N=rec['L2_N'],
                f_NN_target_from_e=rec['f_NN_target_from_e'],
                # Scalar metadata
                seed_t=np.float64(t_seed),
    ...
    manifest = dict(
        ...
        dtype=args.dtype,
        has_forcing=bool(F_phys is not None),
