...
19. **Method doctrine is binding (Sanaa order 2026-07-23): read `docs/LAB_DOCTRINE.md` at session start.** Its load-bearing clauses: field plots accompany every verdict; ALL applicable diagnostics run before any conclusion or new implementation; ONE change at a time (bundled changes declared + ablation scheduled); numerics/artifact triage before physics interpretation (boundary values first; NaN = bug-vs-blowup determination before action); input completeness before training (enumerate every physically plausible input, justify exclusions — the missed-ψ lesson); literature protocol on anything new (subagent shortlists into `docs/papers/`, the supervisor personally reads and judges; state explicitly how our method differs from prior art).
20. **Every resolved error/problem gets an entry in `docs/INCIDENT_LOG.md` in the same session** (symptom → root cause → action → standing rule). An unlogged fix is an unfinished fix. Recurring classes get promoted to a rule here or a hook.
