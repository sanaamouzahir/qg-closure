...
class SpatialGrad(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        dx = F.conv2d(xp, self.wx.to(x.dtype), groups=self.channels)
        dy = F.conv2d(xp, self.wy.to(x.dtype), groups=self.channels)
        return dx, dy
...
class CheapDerivClosureNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, dt: torch.Tensor = None) -> torch.Tensor:
        nt = self.n_time
        ...
        dx, dy = self.grad(allf)
        wx, px = dx[:, :nt], dx[:, nt:]   # omega-x grads, psi-x grads
        wy, py = dy[:, :nt], dy[:, nt:]
