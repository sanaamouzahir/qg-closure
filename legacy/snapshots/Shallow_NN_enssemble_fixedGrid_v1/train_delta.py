...
def physics_part(model, x, regime, der, lap, betasym, scheme, orders, adtype, pure=False):
    ...
    nd = model(x, dt=dT)                              # (B, out_orders, H, W)
    if pure:
...
def main():
    ...
    # ---- resolve roots, restrict to one grid ----
    ...
    target = args.grid or max(grids, key=lambda g: len(grids[g]))
    roots = grids[target]
    print(f"[delta-train] grid={target}  roots={len(roots)}  "
          f"(dropped {sum(len(v) for k, v in grids.items() if k != target)} off-grid)")
    ...
    man0 = train_ds.subsets[0].man
    adtype = torch.float64 if args.compute_dtype == 'float64' else torch.float32
    der, lap, betasym, (Ny, Nx) = build_grid_ops(man0, args.device)
    lap = lap.to(adtype); betasym = betasym.to(adtype)
    dx = float(man0['Lx']) / Nx; dy = float(man0['Ly']) / Ny
    ...
    model = build_model('cheap_deriv', in_channels=in_ch, out_orders=args.out_orders,
                        ...
                        dx=dx, dy=dy, physics_init=not args.no_physics_init,
    ...
    (run_dir / 'config.json').write_text(json.dumps(vars(args) | {'grid': target,
    ...
    for ep in range(args.epochs):
        ...
        if improved:
            ...
            torch.save({'model': model.state_dict(), 'corrector': corrector.state_dict(),
                        'epoch': ep, 'val': va, 'config': vars(args), 'grid': target},
                       run_dir / 'best.pt')
        torch.save({'model': model.state_dict(), 'corrector': corrector.state_dict(),
